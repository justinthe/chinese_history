# Phase 02 — UI shell with mocks · Middle Kingdom Explorer
...
- Every screen in storyboard.md, hooked to hard-coded mock data (the ~28 events / 14 eras from mockup.html moved into `src/mock-data.js`)
- Module skeleton per architecture §1: `router.js`, `landing.js`, `explore.js`, `about.js`, `timeline.js`, `map.js`, `detail.js`, `tour.js`, `search.js` — each `mount(el)` renders its DOM from mock data, interactions call stub functions that visibly do something (navigate, toast)
- Responsive layout down to 360px, including the storyboard's flagged mobile fix: on < 720px the Grand Tour panel becomes a bottom sheet over the timeline instead of squeezing the map
- Run `/impeccable init` once the shell renders (if installed) to capture PRODUCT.md / DESIGN.md
Reference: PRD.md §5 F1–F7, F9, §6 flows 1–9; architecture.md §1, §4 internal module interfaces; storyboard.md screens 1–7.

## Scope Boundary
- In scope:
  - Porting mockup.html's markup, CSS and interaction into ES modules — reuse it, do not redesign it
  - All transitions in the storyboard (screen → screen), with mock behaviour
  - Empty and error states listed in storyboard screens 2, 4, 6
- Out of scope — do NOT touch:
  - Real state store and URL hash sync (phase 03) — a plain module-level object is fine for now
  - Loading JSON from `content/` (phase 03)
  - Collision layout on timeline, morph tweening details, focus trap — those phases replace the mock versions
  - Image loading — use category icon placeholders
...
## Visual Reference
Match these captures exactly (layout, palette, type, spacing):
- images/screen-01-landing.png — Landing
- images/screen-02-explore.png — Explore
- images/screen-03-about.png — About
- images/screen-04-event-detail.png — Event detail panel
- images/screen-05-grand-tour.png — Grand Tour docked beside map
- images/screen-06-search.png — Search dropdown
- images/screen-07-explore-mobile.png — Mobile explore (improve the tour-panel-vs-map gap noted in storyboard.md Screen 7; everything else matches)
Cross-reference storyboard.md for each screen's states and transitions.
...
## Deliverables
- `src/router.js`, `src/views/*.js` (one per module above), `src/mock-data.js`, `src/styles.css` grown from phase 01
- e2e: `e2e/shell.spec.js` walks storyboard Journey 1, 2, 3, 9 on mock data
- PRODUCT.md / DESIGN.md if impeccable is installed
plus `scripts/test-phase-02.sh` (see Manual Test Script below).

## Manual Test Script (mandatory, every phase)
Write `scripts/test-phase-02.sh`, executable, that builds/runs what this
...
3. Run `scripts/test-phase-02.sh` yourself to confirm it works, then tell
