# Phase 05 — Map · Middle Kingdom Explorer
...
- `src/views/map.js` finished per PRD F3: inline SVG from `content/map-shapes.json` (land, korea, japan, rivers, per-era 8-point territory polygons)
- `morphTo(shapeKey, color)` tween 600 ms ease-out-cubic via rAF; instant swap when `prefers-reduced-motion`
- Capital star at `era.capitalXY`; era label card (name, hanzi, oneLiner, capital); year badge
- Pins for `eventsIn(eraId, cats)`; icon by category; click → `detail.open(id)`; keyboard focusable with `aria-label`
- Empty state text when no pins (storyboard screen 2); pin de-overlap: pins within 24 px are nudged apart
- Map zoom +/- buttons: viewBox scale 1×–2× centred on territory centroid
Reference: PRD.md §5 F3, §8 Accessibility (reduced motion, colour not sole signal); architecture.md §3 MapShape, §4 `map.js`; storyboard.md screen 2 (map pane) and screen 5 (morph on tour step).

## Scope Boundary
- In scope:
  - Everything inside the map pane
- Out of scope — do NOT touch:
  - Timeline, detail, tour, search internals
  - Geographic accuracy (PRD A4)
...
## Visual Reference
images/screen-02-explore.png (Qin territory, three pins, capital star) and images/screen-05-grand-tour.png (Xia territory, legendary era label). Territory colours come from `era.color`.
...
## Deliverables
- `src/views/map.js`, `src/lib/tween.js` (pure, unit-tested)
- vitest: `tests/tween.test.js` (interpolation endpoints, reduced-motion path), `tests/map-pins.test.js` (filtering, de-overlap)
- e2e: `e2e/map.spec.js` — scrub from Qin to Han, assert polygon points changed; pin click opens detail
plus `scripts/test-phase-05.sh` (see Manual Test Script below).

## Manual Test Script (mandatory, every phase)
Write `scripts/test-phase-05.sh`, executable, that builds/runs what this
...
3. Run `scripts/test-phase-05.sh` yourself to confirm it works, then tell
