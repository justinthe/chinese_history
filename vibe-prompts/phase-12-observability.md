# Phase 12 — Observability · Middle Kingdom Explorer
...
- `src/diag.js` per architecture §6: `install()` hooks `window.onerror` and `unhandledrejection` → `console.error` of structured JSON `{ts, sessionId, screen, year, eventId, msg, stack}`; image `onerror` → `{ts, eventId, src}` and icon fallback; `setReporter(fn)` integration point (document how to plug Sentry)
- Connectivity check: `navigator.onLine` + failed `content/*.json` fetch → friendly full-screen error with retry (this is the pure-frontend substitute for /health); JSON responses cached via `Cache-Control` headers in the Pages config and a `stale-while-revalidate` fetch wrapper (no service worker unless measured necessary)
- Build-time: validate, fetch-images, coverage each print one JSON line per problem plus a human summary; CI uploads `content/images.report.json` as an artifact
- Analytics: Plausible script tag injected only when `VITE_ANALYTICS=1`; events `event_open`, `tour_start`, `tour_finish`, `search`; CSP `connect-src`/`script-src` extended only under the flag
Reference: architecture.md §6 Observability Plan, §7 CSP; PRD.md §8 Observability, Assumption A6.

## Scope Boundary
- In scope:
  - diag module, offline/error screen, build report format, analytics flag
- Out of scope — do NOT touch:
  - Any third-party error service account setup (reporter stays a no-op function)
  - Service worker / offline content bundle
...
## Visual Reference
New state: the offline/error screen. Follow the landing page's typography and paper background from images/screen-01-landing.png; one illustration, one sentence, one Retry button.
...
## Deliverables
- `src/diag.js`, `src/views/offline.js`, `src/lib/fetch-json.js`
- vitest: `tests/diag.test.js` (structured shape, reporter called), `tests/fetch-json.test.js` (failure path)
- e2e: `e2e/offline.spec.js` (route block content JSON → error screen → unblock → Retry works)
- `docs/observability.md`: how to read logs, plug a reporter, enable analytics
plus `scripts/test-phase-12.sh` (see Manual Test Script below).

## Manual Test Script (mandatory, every phase)
Write `scripts/test-phase-12.sh`, executable, that builds/runs what this
...
(`http://localhost:5173`). Also run `VITE_ANALYTICS=1 npm run build` and show the injected tag, then the default build without it, before the URL. Script must `set -e`, fail loudly
...
```
Manual Checklist — Phase 12 Observability
[ ] Throw an error in console (`setTimeout(()=>{throw new Error('x')})`) → one structured JSON console.error line with sessionId, screen, year
[ ] Break an image src in the manifest → icon fallback + structured log
[ ] DevTools offline → reload → error screen with Retry; go online → Retry restores explore
[ ] Default build has no Plausible tag; VITE_ANALYTICS=1 build has it and CSP allows it
[ ] CI artifact images.report.json is uploaded
```
...
3. Run `scripts/test-phase-12.sh` yourself to confirm it works, then tell
